...
def reference():
    ...
    if not data.get("url", False) or not data.get("email", False) :
        return "Please fill in the inputs of the footer"
...
def truncate_url(link):
    """
    Truncate link to fit in result format.
    """
    link = parse.unquote(link)
    if len(link) <= 60 :
        return link
    link = link[:-1] if link.endswith("/") else link
    link = link.split("//",1)[1].split("/")
    link = "%s/.../%s"%(link[0],link[-1])
    return link[:60]+"..." if len(link) > 60 else link