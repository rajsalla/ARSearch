...
class LabeledThreadReader:
    def __init__(self, content, thread_id, title_dict, tag_dict):
        ...
        self.texts = self.remove_labels(self.texts_w_labels, "text")
        self.code_blocks = self.remove_labels(self.code_blocks_w_labels, "code")
    ...
    def _tokenize(self, content):
        ...
        return tokens
    def remove_labels(self, content, content_type):
        ...
        for par in content:
            ...
            while match is not None:
                ...
                if len(api_mention_tokens) > 0:
                    ...
                    simple_name_idx_in_api_mention_tokens = len(api_mention_tokens)-1

                    api_pos =len(before_api_tokens) +simple_name_idx_in_api_mention_tokens
                    simple_name, api_pos
                    list_labeled_simple_names.append(simple_name)
                    list_labels.append(label)
                    list_labeled_token_pos.append(api_pos)
            ...
            api_mentions += zip(list_labeled_simple_names, list_labels, list_thread_ids, list_labeled_token_pos, list_paragraphs, content_types)
    ...
    def find_text_with_regex(self, content):
        ...
        texts = [text_line.strip("\n").strip() for text_line in texts if text_line.strip("\n") != ""]
        return texts
